...
async def create_users(body: UserSchema, db: Session) -> User:
    user_ = User(first_name=body.first_name,last_name=body.last_name,birthday_date = body.birthday_date,emails=body.emails,phone_numbers=body.phone_numbers,other_description = body.other_description)
    db.add(user_)
    db.commit()
    db.refresh(user_)
    return user_
